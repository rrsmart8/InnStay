...
# POST guest booking (fără autentificare)
@bookings_bp.route("/guest", methods=["POST"])
def create_guest_reservation():
    data = request.get_json()
    required_fields = ["room_id", "guest_name", "guest_email", "guests", "check_in_date", "check_out_date"]
    ...
    try:
        ...
        if not guest_user:
            return jsonify({"error": "Guest user not found in database."}), 500
        room_id = data["room_id"]
        check_in = datetime.strptime(data["check_in_date"], "%Y-%m-%d")
        check_out = datetime.strptime(data["check_out_date"], "%Y-%m-%d")
        overlapping = Booking.query.filter(
